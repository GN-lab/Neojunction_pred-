#!/usr/bin/env Rscript
# Step 15c: Map ALT n-mers back to neojunctions
...
#   wt_native_tier1_YYYYMMDD.tsv           -- native WT Tier 1 candidates;
#                                             deliberately not NJ-mapped
...
###########################################################################
#  Step 4: Preserve native WT Tier 1 without neojunction mapping
###########################################################################

cat("\n[STEP 4] Loading native WT Tier 1 (no neojunction restriction)...\n")
...
if (file.exists(wt_file)) {
  ...
  cat(sprintf("[INFO] WT Tier 1 candidates retained: %d rows | %d peptides\n",
              nrow(wt_t1), uniqueN(wt_t1$peptide)))
  wt_result <- list(map = wt_t1, nj_summary = data.table())
} else {
...
if (nrow(wt_result$map) > 0) {
  fwrite(wt_result$map,
         paste0("wt_native_tier1_", current_date, ".tsv"),
         sep = "\t", na = "NA", quote = FALSE)
  cat(sprintf("[INFO] WT native: %d rows -> wt_native_tier1_%s.tsv\n",
...
cat(sprintf("  WT native Tier1 rows:       %d (not NJ-restricted)\n",
            nrow(wt_result$map)))
